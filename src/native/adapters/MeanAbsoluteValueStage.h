...
namespace dsp::adapters
{
    ...
    public:
         ...
         */
        explicit MeanAbsoluteValueStage(MavMode mode, size_t window_size = 0)
            : m_mode(mode), m_window_size(window_size)
        {
            if (m_mode == MavMode::Moving && window_size == 0)
            {
                throw std::invalid_argument("MeanAbsoluteValue: window size must be greater than 0 for 'moving' mode");
            }
        }
