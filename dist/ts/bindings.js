...
if (!DspAddon) {
    ...
    throw new Error(`Could not load native module. Tried ${possiblePaths.length} paths.`);
}
/**
...
class DspProcessor {
     ...
     */
    generateLogTopic(level, context) {
    ...
    MovingAverage(params) {
        if (params.mode === "moving") {
            if (params.windowSize === undefined &&
                ...
                throw new TypeError(`MovingAverage: either windowSize or windowDuration must be specified for "moving" mode`);
            }
            if (params.windowSize !== undefined &&
                (params.windowSize <= 0 || !Number.isInteger(params.windowSize))) {
                throw new TypeError(`MovingAverage: windowSize must be a positive integer for "moving" mode, got ${params.windowSize}`);
            }
            if (params.windowDuration !== undefined && params.windowDuration <= 0) {
                throw new TypeError(`MovingAverage: windowDuration must be positive, got ${params.windowDuration}`);
            }
        }
    ...
    Rms(params) {
        if (params.mode === "moving") {
            if (params.windowSize === undefined &&
                params.windowDuration === undefined) {
                throw new TypeError(`RMS: either windowSize or windowDuration must be specified for "moving" mode`);
            }
            if (params.windowSize !== undefined &&
                (params.windowSize <= 0 || !Number.isInteger(params.windowSize))) {
                throw new TypeError(`RMS: windowSize must be a positive integer for "moving" mode, got ${params.windowSize}`);
            }
            if (params.windowDuration !== undefined && params.windowDuration <= 0) {
                throw new TypeError(`RMS: windowDuration must be positive, got ${params.windowDuration}`);
            }
        }
    ...
    Variance(params) {
        if (params.mode === "moving") {
            if (params.windowSize === undefined &&
                params.windowDuration === undefined) {
                throw new TypeError(`Variance: either windowSize or windowDuration must be specified for "moving" mode`);
            }
            if (params.windowSize !== undefined &&
                (params.windowSize <= 0 || !Number.isInteger(params.windowSize))) {
                throw new TypeError(`Variance: windowSize must be a positive integer for "moving" mode, got ${params.windowSize}`);
            }
            if (params.windowDuration !== undefined && params.windowDuration <= 0) {
                throw new TypeError(`Variance: windowDuration must be positive, got ${params.windowDuration}`);
            }
        }
     ...
     */
    ZScoreNormalize(params) {
        if (params.mode === "moving") {
            if (params.windowSize === undefined &&
                params.windowDuration === undefined) {
                throw new TypeError(`Z-Score Normalize: either windowSize or windowDuration must be specified for "moving" mode`);
            }
            if (params.windowSize !== undefined &&
                (params.windowSize <= 0 || !Number.isInteger(params.windowSize))) {
                throw new TypeError(`Z-Score Normalize: windowSize must be a positive integer for "moving" mode, got ${params.windowSize}`);
            }
            if (params.windowDuration !== undefined && params.windowDuration <= 0) {
                throw new TypeError(`Z-Score Normalize: windowDuration must be positive, got ${params.windowDuration}`);
            }
        }
     ...
     */
    MeanAbsoluteValue(params) {
        if (params.mode === "moving") {
            if (params.windowSize === undefined &&
                params.windowDuration === undefined) {
                throw new TypeError(`Mean Absolute Value: either windowSize or windowDuration must be specified for "moving" mode`);
            }
            if (params.windowSize !== undefined &&
                (params.windowSize <= 0 || !Number.isInteger(params.windowSize))) {
                throw new TypeError(`Mean Absolute Value: windowSize must be a positive integer for "moving" mode, got ${params.windowSize}`);
            }
            if (params.windowDuration !== undefined && params.windowDuration <= 0) {
                throw new TypeError(`Mean Absolute Value: windowDuration must be positive, got ${params.windowDuration}`);
            }
        }
     ...
     */
    WaveformLength(params) {
        if (params.windowSize <= 0 || !Number.isInteger(params.windowSize)) {
            throw new TypeError(`WaveformLength: windowSize must be a positive integer, got ${params.windowSize}`);
        }
     ...
     */
    SlopeSignChange(params) {
        if (params.windowSize <= 0 || !Number.isInteger(params.windowSize)) {
            throw new TypeError(`SlopeSignChange: windowSize must be a positive integer, got ${params.windowSize}`);
        }
        if (params.threshold !== undefined && params.threshold < 0) {
            throw new TypeError(`SlopeSignChange: threshold must be non-negative, got ${params.threshold}`);
        }
     ...
     */
    WillisonAmplitude(params) {
        if (params.windowSize <= 0 || !Number.isInteger(params.windowSize)) {
            throw new TypeError(`WillisonAmplitude: windowSize must be a positive integer, got ${params.windowSize}`);
        }
        if (params.threshold !== undefined && params.threshold < 0) {
            throw new TypeError(`WillisonAmplitude: threshold must be non-negative, got ${params.threshold}`);
        }
    ...
    filter(options) {
        ...
        throw new Error("Filter stages in pipeline not yet implemented in C++ layer. Use standalone filters with manual chaining instead:\n" +
     ...
     */
    createFirFilter(options) {
        const { mode, cutoffFrequency, lowCutoffFrequency, highCutoffFrequency } = options;
        switch (mode) {
            case "lowpass":
                if (!cutoffFrequency) {
                    throw new Error("cutoffFrequency required for lowpass filter");
                }
                return FirFilter.createLowPass(options);
            case "highpass":
                if (!cutoffFrequency) {
                    throw new Error("cutoffFrequency required for highpass filter");
                }
                return FirFilter.createHighPass(options);
            case "bandpass":
                ...
                return FirFilter.createBandPass(options);
            case "bandstop":
            case "notch":
                if (!lowCutoffFrequency || !highCutoffFrequency) {
                    throw new Error("lowCutoffFrequency and highCutoffFrequency required for bandstop filter");
                }
                return FirFilter.createBandStop(options);
            default:
     ...
     */
    createButterworthFilter(options) {
        const { mode, cutoffFrequency, lowCutoffFrequency, highCutoffFrequency } = options;
        switch (mode) {
            case "lowpass":
                if (!cutoffFrequency) {
                ...
                return IirFilter.createButterworthLowPass(options);
            case "highpass":
                if (!cutoffFrequency) {
                    throw new Error("cutoffFrequency required for highpass filter");
                }
                return IirFilter.createButterworthHighPass(options);
            case "bandpass":
                if (!lowCutoffFrequency || !highCutoffFrequency) {
                    throw new Error("lowCutoffFrequency and highCutoffFrequency required for bandpass filter");
                }
                return IirFilter.createButterworthBandPass(options);
            default:
     ...
     */
    createChebyshevFilter(options) {
        const { mode, cutoffFrequency, lowCutoffFrequency, highCutoffFrequency, ripple = 0.5, } = options;
        switch (mode) {
            case "lowpass":
                if (!cutoffFrequency) {
                    throw new Error("cutoffFrequency required for lowpass filter");
    ...
    createBiquadFilter(options) {
        const { mode, cutoffFrequency, q = 0.707, gain = 0 } = options;
        ...
        switch (mode) {
            ...
            case "bandstop":
            case "notch":
                throw new Error(`Biquad ${mode} filters not yet implemented. Use Butterworth or Chebyshev filters instead.`);
            default:
