    ...
    format(log) {
        ...
        if (log.context && Object.keys(log.context).length > 0) {
            try {
                ...
                output += `\n  Context: [Unable to stringify: ${error instanceof Error ? error.message : "unknown error"}]`;
            }
        }
        return output;
    }
 ...
 */
export function withTracingContext(context, fn) {
 ...
 */
async function retryWithBackoff(fn, maxAttempts = 3, onRetry) {
    let lastError;
 ...
 */
export function generateTraceparent(traceId, spanId) {
    if (!traceId || !spanId)
        return undefined;
 ...
 * @returns Response
 */
async function postJSON(url, body, headers, method = "POST", log) {
    const finalHeaders = {
 ...
 */
export function createPagerDutyHandler(config) {
    const { endpoint, apiKey } = config;
    const severityMap = SEVERITY_MAPPINGS.pagerduty;
    if (!endpoint || !apiKey) {
        console.warn("PagerDuty handler: endpoint or apiKey (routing_key) not configured. Logs will be dropped.");
    }
    return async (log) => {
        if (!endpoint || !apiKey) {
            return;
            ...
            await retryWithBackoff(async () => {
                const payload = {
                    ...
                    payload: {
                        ...
                        timestamp: new Date(normalizeTimestamp(log.timestamp, "ms")).toISOString(),
                        custom_details: {
                            ...log.context,
                            traceId: log.traceId,
                            spanId: log.spanId,
                ...
                try {
                    ...
                    if (error instanceof Error && error.message.includes("401")) {
                        throw new Error(`PagerDuty authentication failed: Invalid routing_key. Check your Integration Key.`);
                    }
                    throw error;
                }
            }, 3, 
 ...
 */
export function createPrometheusHandler(config) {
    const { endpoint } = config;
    if (!endpoint) {
        console.warn("Prometheus handler: endpoint not configured. Metrics will be dropped.");
    }
    return async (log) => {
        if (!endpoint) {
            return;
        ...
        await concurrencyLimiter.run(async () => {
            await retryWithBackoff(async () => {
                ...
                const metric = `${metricName}{${labels},schema="${SCHEMA_VERSION}"} ${log.context?.value || 1} ${timestamp}`;
                try {
                    ...
                    if (!response.ok) {
                        if (response.status === 401 || response.status === 403) {
                            throw new Error(`Prometheus authentication failed (${response.status}). Check config.headers for HTTP Basic Auth credentials.`);
                        }
                        throw new Error(`HTTP ${response.status} - ${response.statusText}`);
                    }
                }
                    ...
                    if (error instanceof Error &&
                        ...
                        throw new Error(`Prometheus Pushgateway unreachable at ${endpoint}. Check network and endpoint.`);
                    }
                    throw error;
                }
            });
 ...
 * @throws Error if authentication fails or endpoint is unreachable
 */
export function createLokiHandler(config) {
    const { endpoint, apiKey } = config;
    const buffer = [];
    let flushTimer = null;
    if (!endpoint) {
        console.warn("Loki handler: endpoint not configured. Logs will be buffered but never sent.");
    }
    const flush = async () => {
        if (buffer.length === 0)
            return;
                ...
                try {
                    ...
                    if (error instanceof Error) {
                        if (error.message.includes("401") ||
                            ...
                            throw new Error(`Loki authentication failed. Check apiKey (Bearer token) or config.headers for Basic Auth / X-Scope-OrgID.`);
                        }
                    }
                    throw error;
                }
 ...
 */
export function createCloudWatchHandler(config) {
    const { endpoint, apiKey } = config;
    if (!endpoint || !apiKey) {
        console.warn("CloudWatch handler: endpoint or apiKey not configured. " +
                ...
                try {
                    ...
                    if (error instanceof Error) {
                        if (error.message.includes("403") ||
                            ...
                            throw new Error(`CloudWatch authentication failed. AWS requires proper IAM credentials and request signing. ` +
 ...
 */
export function createDatadogHandler(config) {
    const { endpoint = "https://http-intake.logs.datadoghq.com", apiKey } = config;
    const severityMap = SEVERITY_MAPPINGS.datadog;
    if (!apiKey) {
        console.warn("Datadog handler: apiKey not configured. Logs will be dropped.");
    }
    return async (log) => {
        if (!apiKey) {
            return;
        ...
        await concurrencyLimiter.run(async () => {
            await retryWithBackoff(async () => {
                ...
                try {
                    await postJSON(`${endpoint}/v1/input/${apiKey}`, payload, config.headers);
                }
                catch (error) {
                    if (error instanceof Error) {
                        if (error.message.includes("403") ||
                            error.message.includes("401")) {
                            throw new Error(`Datadog authentication failed. Check apiKey validity and regional endpoint (US vs EU). Error: ${error.message}`);
                        }
                    }
                    throw error;
                }
...
export function createConsoleHandler(config = {}) {
    ...
    return (log) => {
        ...
        const timestamp = new Date(log.timestamp).toISOString();
        const traceInfo = log.traceId ? ` [trace:${log.traceId.slice(0, 8)}]` : "";
        console.log(`${color}[${timestamp}] [${log.level.toUpperCase()}] [${log.topic || "unknown"}]${traceInfo}${colors.reset} ${log.message}`);
        if (log.context && Object.keys(log.context).length > 0) {
 ...
 */
export function createMockHandler(onLog) {
    const logs = [];
    const handler = (log) => {
        logs.push(log);
        if (onLog) {
        ...
        await Promise.all(this.handlers.map(async (handler) => {
            try {
                ...
                if (this.enableMetrics) {
                    ...
                    this.metrics.handlerErrors.set(handlerName, (this.metrics.handlerErrors.get(handlerName) || 0) + 1);
                }
